...
while read ORDER_ID
do
echo select orders.date_purchased, orders_products.products_model, orders_products.products_name, orders_products.products_price, orders_products_attributes.products_options_values from orders_products,orders, orders_products_attributes where orders_products.orders_id = orders.orders_id AND  orders_products.orders_products_id = orders_products_attributes.orders_products_id AND orders_products.orders_id = $ORDER_ID\;
